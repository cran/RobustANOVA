RGF <- function(formula, data, alpha = 0.05, verbose = TRUE, p_shape, repn=5000){
  ...
  for(i in 1:repn){
    ui <- rchisq(a,n-1);
    vi_square <- (n-1)*(sigmahat^2)
    TN <- sum((bigm*ui*(muhat^2))/(vi_square))-(((sum((bigm*ui*muhat)/(vi_square)))^2)/(sum((bigm*ui)/vi_square)))
    U <- rchisq(1,a-1)
    if((U/TN)>1){pv=pv+1}
  }
  pvalue <- pv/repn;
  ...
  if (verbose) {
    cat("\n", "","Robust Generalized F Test based on MML Estimators", paste("(alpha = ",alpha,")",sep = ""), "\n", 
        sep = " ")
    cat({"-------------------------------------------------------------"}, 
        "\n", sep = " ")
  ...
  result$alpha <- alpha
  result$method <- "Robust Generalized F Test based on MML Estimators"
  result$data <- data
